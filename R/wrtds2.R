######
#' Get WRTDS prediction grid using rollapply
#'
#' Get WRTDS prediction grid for chlorophyll observations in a tidal or tidalmean object, uses \code{\link[zoo]{rollapply}} from zoo
#'
...
#' 
#' ## multiple quantiles
#' res <- wrtds2(dat_in, tau = c(0.1, 0.5, 0.9))
#' }
wrtds2 <- function(dat_in, ...) UseMethod('wrtds2')
...
#'
#' @method wrtds2 tidal
wrtds2.tidal <- function(dat_in, sal_div = 10, tau = 0.5, trace = TRUE, ...){
...
#' @export
#'
#' @method wrtds2 tidalmean
wrtds2.tidalmean <- function(dat_in, sal_div = 10, trace = TRUE, ...){
  ...
  dat_in <- zoo(dat_in, dat_in$date)
  
  # iterate through rows of dat_in
  rollapply(dat_in, width = nrow(dat_in), FUN = wrtdsroll, align = 'center', by.column = FALSE)

...
#' 
#' @return The original data frame with additional columns for a flow-independent water quality time series (\code{lnres_noQ}) and a flow-dependent time series (\code{lnres_Q}).
#' 
#' @export
#' 
...
lnres_sim <- function(dat_in, lnQ_coef = NULL){
  ...
  lnres_noQ <- lm(lnres ~ dec_time + sin(2 * pi * dec_time) + cos(2 * pi * dec_time), 
    data = dat_in)
  lnres_noQ <- predict(lnres_noQ) + with(dat_in, scls * errs)
